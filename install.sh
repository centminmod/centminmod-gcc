...
GCC_SNAPSHOTNINE='https://bigsearcher.com/mirrors/gcc/snapshots/LATEST-9/'
#GCC_SNAPSHOTNINE='http://mirrors.concertpass.com/gcc/snapshots/9-20191207/'
GCC_SNAPSHOTTEN='https://bigsearcher.com/mirrors/gcc/snapshots/LATEST-10/'
...
binutils_install() {
    ...
    if [[ "$GCC_SVN" = [yY] && "$GCCSVN_VER" -eq '7' ]]; then
        ...
        downloadtar_dirname=$(echo "$downloadtar_name" | sed -e 's|.tar.xz||')
        GCC_PREFIX="/opt/${downloadtar_dirname}"
    elif [[ "$GCC_SVN" = [yY] && "$GCCSVN_VER" -eq '10' ]]; then
        GCC_SYMLINK='/opt/gcc10'
        downloadtar_name=$(curl -4s $GCC_SNAPSHOTTEN | grep -o '<a .*href=.*>' | grep -v '.sig' | sed -e 's/<a /\n<a /g' | sed -e 's/<a .*href=['"'"'"]//' -e 's/["'"'"'].*$//' -e '/^$/ d' -e 's|\.\.\/||g' -e 's|\.\/||g'| awk '/tar.xz/ {print $1}')
        downloadtar_dirname=$(echo "$downloadtar_name" | sed -e 's|.tar.xz||')
        GCC_PREFIX="/opt/${downloadtar_dirname}"
    fi
...
install_gcc() {
    ...
    if [[ "$GCC_SVN" = [nN] ]]; then
        ...
        rm -rf "${downloadtar_dirname}"
        rm -rf "${downloadtar_name}"
        echo "wget -4 "$GCC_SNAPSHOTTEN/${downloadtar_name}""
        wget -4 "$GCC_SNAPSHOTTEN/${downloadtar_name}"
        echo "tar xf ${downloadtar_name}"
        tar xf "${downloadtar_name}"
        cd "$downloadtar_dirname"
        if [[ "$GCC_DOWNLOADPREREQ" = [yY] ]]; then
...
case "$1" in
    ...
    * )
        ...
        echo "Usage:"
        echo
        echo "$0 {install|install7|install8|install9|install10|installpgo7|installpgo8|installpgo9|installpgo10|installgcc|installgcc7|installgcc8|installgcc9|installgcc10|installpgogcc7|installpgogcc8|installpgogcc9|installpgogcc10|binutils7|binutils8|binutils9|binutils10}"
        ;;
esac
